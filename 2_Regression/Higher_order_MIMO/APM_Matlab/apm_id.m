...
if (~fast),
    ...
    plot(time,ypred(:,1),'g--','LineWidth',1);
    plot(time,ypred(:,2),'b-','LineWidth',1);
    legend('Temperature 1 (Measured)', 'Temperature 2 (Measured)',...
        'Temperature 1 (Predicted)', 'Temperature 2 (Predicted)')
    ylabel('Temp (degC)')
    xlabel('Time (sec)')
    axis tight
